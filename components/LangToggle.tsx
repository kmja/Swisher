...
export default function LangToggle({ lang, onChange }: { lang: Lang; onChange: (l: Lang) => void }) {
  return (
    <div className="inline-flex overflow-hidden rounded-full bg-white text-sm font-semibold shadow-sm ring-1 ring-gray-200">
      {(["sv", "en"] as Lang[]).map((l) => (
        <button
          key={l}
          type="button"
          aria-pressed={lang === l}
          onClick={() => onChange(l)}
          className={`px-3 py-2 ${lang === l ? "bg-swish text-white" : "text-gray-500 active:bg-gray-100"}`}
        >
          {l.toUpperCase()}
        </button>
      ))}
