...
- Clean up each description into the real menu item. Receipts are often misread or abbreviated, so correct obvious OCR errors and misspellings to the word that was clearly intended — fix dropped, swapped or wrong letters (e.g. "ryggkaffe" → "Bryggkaffe", "gstron"/"östro" → "Ostron", "entrecöte"/"entrecote" → "Entrecôte", "vamkorv" → "Varmkorv", "ceasarsallad" → "Caesarsallad") and use normal capitalisation, not ALL CAPS.
...
- The receipt is internally consistent, so use its own numbers to verify yours: on a multi-unit line, quantity × per-unit price equals the line total; and all line totals add up to the printed grand total ("Tot Kvitto", "Totalt", "Summa", or the amount after "SEK"). When a price is faint or ambiguous, pick the reading that keeps the line consistent AND makes every line total sum to that grand total. Read the grand total ("total") itself with extra care, since it anchors all the other numbers.
