...
- Transcribe each item exactly as printed. You may fix an obvious OCR slip — a single dropped, swapped, or wrong letter (e.g. "ryggkaffe" → "Bryggkaffe", "gstron" → "Ostron", "entrecote" → "Entrecôte") — and use normal capitalisation instead of ALL CAPS. But do NOT replace a faint or unclear word with a different, more plausible-sounding dish: if you cannot read it confidently, give your best literal letter-by-letter reading. Never invent items, lines, or prices that are not printed on the receipt — it is better to leave a description rough than to guess a different menu item.
